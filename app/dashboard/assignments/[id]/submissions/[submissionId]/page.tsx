...
export default function SubmissionReviewPage() {
  ...
  const updateComment = (id: string, text: string) => {
    const updatedComments = inlineComments.map((comment) =>
  ...
  const scrollToComment = (commentId: string) => {
    ...
    if (commentSidebarRef.current) {
      const commentElement = commentSidebarRef.current.querySelector(
  ...
  const renderHighlightedContent = () => {
    ...
    for (const segment of segments) {
      if (segment.type === 'text') {
        ...
        for (let i = 1; i < (paragraphTexts?.length ?? 0); i++) {
  ...
  const saveChanges = async () => {
    try {
      await api.put(`/submission/edit/${submissionId}`, {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" asChild>
            <Link href={`/dashboard/assignments/${id as string}`}>
            ...
            <TabsContent value="ai">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Confidence: {submission?.aiCheckerResults?.confidence}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
